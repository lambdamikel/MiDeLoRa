...
(setf (logical-pathname-translations "lubm")
      '(("**;*.*.*" "dlmaps:prover;lubm;**;*.*")))

(setf (logical-pathname-translations "results")
      '(("**;*.*.*" "dlmaps:prover;results;**;*.*")))
...
(defun load-kbs (&optional (univs 1) (departments 1))
  ...
  (let ((name (if (= univs 0)
    ...
    (let ((*package* (find-package :prover)))
      (load (format nil "lubm:~A.tbox" name))
      (load (format nil "lubm:~A.abox" name)))
        ...
        (let ((ts::*multiprocess-queries* nil)
          ...
          (dolist (setting (list
            ...
            (with-open-file (*file-stream* 
                             (format nil "results:result-univs-~A-deps-~A-setting-~A.res" 
                                     univs
                                     deps
                                     settings)
                             :direction :output 
  ...
  (let ((ts::*multiprocess-queries* nil))
    ...
    (thematic-substrate::start-process 
     ...
     (let ((ts::*multiprocess-queries* nil))
       ...
       (with-open-file (*file-stream* 
			"results:subsumption-test-midelora-result-1.res"
			:direction :output 
			:if-exists :supersede 
			:if-does-not-exist :create)
	 ...
	 (ts::with-nrql-settings (:mode 3
       ...
       (with-open-file (*file-stream* 
                        "results:subsumption-test-midelora-result-2.res"
			:direction :output 
			:if-exists :supersede 
			:if-does-not-exist :create)
